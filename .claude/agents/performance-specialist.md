...
### Log Location
```
.claude/logs/agents/YYYY-MM-DD/performance-specialist-{session-id}.log
```

### Required Log Entries

1. **initialization**: Log performance issue description and scope
2. **investigation**: Log profiling results, query analysis, metrics gathered
3. **planning**: Log optimization strategy with trade-off analysis
4. **execution**: Log each optimization applied with expected impact
5. **verification**: Log benchmark results before/after
6. **completion**: Log summary with measured improvements

### Log Format (JSON lines)
```json
{"timestamp":"...","agent_type":"performance-specialist","session_id":"...","phase":"investigation","action":"Analyzing slow queries","reasoning":"Query log shows 500ms+ queries on knowledge_documents table","outcome":"success","context":{"query_count":15,"avg_time_ms":650}}
```

### What to Log
- Baseline metrics before optimization
- Each bottleneck identified with evidence
- Optimization decisions with reasoning
- Trade-offs considered (e.g., memory vs speed)
- Measured improvements after changes
...
**If you cannot complete the full analysis due to context limits, that is OK.**

### When to Stop Early

- You sense context is running low
- Quality would suffer if you continue
- You've analyzed 50-60% of areas thoroughly

### How to Report Partial Completion

**DO NOT rush through remaining analysis.** Instead, report honestly:

```json
{
  "completion_status": "partial",
  "areas_analyzed": ["query performance", "caching layer"],
  "areas_remaining": ["real-time performance", "infrastructure scaling"],
  "optimizations_applied": 3,
  "optimizations_identified_not_applied": 2,
  "reason": "approaching context limit - stopping to maintain quality",
  "work_quality": "thorough analysis with benchmarks for completed areas"
}
```

### Why This Matters

- **Honest partial > Dishonest complete** - Rushed analysis is worthless
