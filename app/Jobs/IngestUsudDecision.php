...
class IngestUsudDecision implements ShouldQueue
{
    ...
    {
        return 'Ingesting USUD Decision: '.$this->decisionId;
    }

    public function handle(UsudIngestService $ingestService): void
    {
        Log::info('IngestUsudDecision job starting', [
            'decision_id' => $this->decisionId,
            'attempt' => $this->attempts(),
            'max_tries' => $this->tries,
        ]);

        $broadcastJobId = 'ingest_usud_'.$this->decisionId;
        ...
        try {
            ...
            if ($inserted > 0 && $errors === 0) {
                ...
                Log::info('USUD decision ingested successfully', [
                    'decision_id' => $this->decisionId,
                    'inserted' => $inserted,
                ]);
            ...
            $message = $errors > 0 ? 'USUD ingestion reported errors' : 'USUD ingestion produced no chunks';
            if ($this->userId) {
                $this->broadcastFailed($this->userId, $broadcastJobId, $message, 'Ingestion');
            }

            Log::warning('USUD decision ingestion incomplete', [
                'decision_id' => $this->decisionId,
                'inserted' => $inserted,
                'errors' => $errors,
            ]);
            ...
            Log::error('USUD decision ingestion failed', [
                'decision_id' => $this->decisionId,
                'error' => $e->getMessage(),
            ]);
