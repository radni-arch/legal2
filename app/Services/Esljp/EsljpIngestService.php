...
class EsljpIngestService
{
    public function __construct(
        protected EsljpClient $client,
    ...
    public function searchList(string $query, array $options = []): array
    {
    ...
    public function filterAlreadyIngestedIds(array $sourceIds): array
    {
        ...
        if (count($existingIds) > 0) {
            Log::info('Filtered already-ingested ESLJP decision IDs', [
    ...
    public function ingestByIdsQueued(array $ids, array $options = []): array
    {
        ...
        foreach ($ids as $id) {
            ...
            try {
                IngestEsljpDecision::dispatch($id, $options)->onQueue($queueName);
                $queued++;
            } catch (\Throwable $e) {
                $errors++;
                Log::error('Failed to queue ESLJP decision for ingestion', [
                    'decision_id' => $id,
                    'error' => $e->getMessage(),
                ]);
            }
    ...
    protected function ingestByIdsSync(array $ids, array $options = []): array
    {
        ...
        $syncGraph = (bool) ($options['sync_graph'] ?? config('esljp.sync_graph', false));
        ...
        foreach ($ids as $id) {
            ...
            try {
                ...
                if (isset($detail['error'])) {
                    $errors++;
                    Log::warning('ESLJP detail fetch failed', [
                        'id' => $id,
                        'error' => $detail['error'],
                    ]);
                    continue;
                ...
                if (! $pdfUrl) {
                    $errors++;
                    Log::warning('ESLJP decision missing PDF link', ['id' => $id]);
                    continue;
                }
                ...
                if (! ($pdfRes['ok'] ?? false) || ! is_string($pdfRes['bytes'] ?? null)) {
                    $errors++;
                    Log::warning('ESLJP PDF download failed', [
                        'id' => $id,
                ...
                if ($text === '') {
                    $skipped++;
                    Log::warning('ESLJP ingestion skipped due to empty text', ['id' => $id]);
                    continue;
                }
                ...
                if ($syncGraph && ! $dry && $this->graphDb && $currentInserted > 0) {
                    try {
                        ...
                        $graphErrors++;
                        Log::warning('ESLJP graph sync failed (vector ingestion succeeded)', [
                            'id' => $id,
                            'doc_id' => $docId,
                ...
                $errors++;
                Log::warning('ESLJP ingest failed', ['id' => $id, 'error' => $e->getMessage()]);
            }
        }
    ...
    protected function upsertDecisionFromMeta(array $m): CourtDecision
    {
        ...
        $title = trim((string) ($m['title'] ?? '')) ?: 'ESLJP odluka';
        $court = 'ESLJP';
        ...
        $attrs = [
            ...
            'tags' => array_filter([
                ...
                $caseNumber ? 'zahtjev '.$caseNumber : null,
            ]),
            'description' => null,
        ];
    ...
    {
        $tmp = tempnam(sys_get_temp_dir(), 'esljp_');
        $path = $tmp.$suffix;
        @unlink($tmp);
    ...
    {
        return str_starts_with($id, 'esljp:') ? substr($id, 6) : $id;
    }
