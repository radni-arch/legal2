...
class OpenAIService
{
    ...
    protected function client()
    {
        ...
        $retryConfig = config('openai.retry', []);
        if (! empty($retryConfig['times'])) {
            $times = (int) $retryConfig['times'];
            $sleepMs = (int) ($retryConfig['sleep_ms'] ?? 200);
            $exponentialBackoff = (bool) ($retryConfig['exponential_backoff'] ?? true);
            $jitter = (bool) ($retryConfig['jitter'] ?? true);
            $statusCodes = $retryConfig['when']['status_codes'] ?? [408, 429, 500, 502, 503, 504];

            $client = $client->retry(
                $times,
                function (int $attempt, ?\Throwable $exception = null) use ($sleepMs, $exponentialBackoff, $jitter) {
                    $sleep = $sleepMs;

                    if ($exponentialBackoff) {
                        $sleep = $sleepMs * pow(2, $attempt - 1);
                    }

                    if ($jitter) {
                        $sleep += random_int(0, (int) ($sleepMs * 0.5));
                    }

                    return $sleep;
                },
                function (\Throwable $exception, PendingRequest $request) use ($statusCodes) {
                    ...
                    if ($exception instanceof RequestException) {
                        ...
                        if ($response && in_array($response->status(), $statusCodes)) {
                            return true;
                        }
                    }
     ...
     * @throws Throwable
     */
    protected function request(string $method, string $url, array $payload = [], array $options = [])
    {
        ...
        try {
            ...
            $resp = $this->circuitBreaker->call(function () use ($method, $url, $payload, $options) {
    ...
    protected function getWithHeaders(string $url, array $headers = [])
    {
        ...
        try {
            ...
            $resp = $this->circuitBreaker->call(function () use ($url, $headers) {
                return $this->client()->withHeaders($headers)->get(ltrim($url, '/'));
            });
    ...
    public function responses(array $options): array
    {
        ...
        $requestOptions = [];
        $responsesTimeout = (int) config('openai.responses_timeout');
        if ($responsesTimeout > 0) {
            $requestOptions['timeout'] = $responsesTimeout;
        }

        return $this->request('POST', '/responses', $options, $requestOptions);
    }
    ...
    /**
     * Simple completion method that wraps chat with a single user message
     * This is a convenience method for single-turn completions
     *
    ...
    public function complete(string $prompt, array $options = []): array
    {
        ...
        // Extract model if provided in options
    ...
    public function transcribe(string $filePath, array $options = [])
    {
        ...
        try {
            // Wrap multipart upload with circuit breaker
            $resp = $this->circuitBreaker->call(function () use ($headers, $model, $filePath, $params) {
                return Http::withHeaders($headers)
                    ->baseUrl($this->baseUrl)
                    ->timeout($this->timeout)
                    ->connectTimeout($this->connectTimeout)
            ...
            $duration = (int) round((microtime(true) - $start) * 1000);
            $this->logChannel()->warning('openai.transcribe.circuit_breaker', [
                'event' => 'openai.transcribe.circuit_breaker',
                'request_id' => $reqId,
                'duration_ms' => $duration,
    ...
    public function tts(string $text, array $options = []): string
    {
        ...
        try {
            // Wrap TTS request with circuit breaker
            $resp = $this->circuitBreaker->call(function () use ($payload) {
                return $this->client()->asJson()->post('/audio/speech', $payload);
            });
            ...
            // Circuit breaker exception - re-throw as-is
            $duration = (int) round((microtime(true) - $start) * 1000);
            $this->logChannel()->warning('openai.tts.circuit_breaker', [
                'event' => 'openai.tts.circuit_breaker',
                'request_id' => $reqId,
                'duration_ms' => $duration,
                'circuit_state' => $this->circuitBreaker->getStatus()['state'],
            ]);
    ...
    public function fileUpload(string $path, string $purpose = 'assistants'): array
    {
        ...
        try {
            // Wrap multipart file upload with circuit breaker
            $resp = $this->circuitBreaker->call(function () use ($headers, $path, $purpose) {
                return Http::withHeaders($headers)
                    ->baseUrl($this->baseUrl)
                    ->timeout($this->timeout)
                    ->connectTimeout($this->connectTimeout)
            ...
            // Circuit breaker exception - re-throw as-is
            $duration = (int) round((microtime(true) - $start) * 1000);
            $this->logChannel()->warning('openai.file_upload.circuit_breaker', [
                'event' => 'openai.file_upload.circuit_breaker',
                'request_id' => $reqId,
                'duration_ms' => $duration,
                'circuit_state' => $this->circuitBreaker->getStatus()['state'],
            ]);
    ...
    public function assistantsList(array $query = []): array
    {
        ...
        $url = '/assistants'.($queryString ? '?'.$queryString : '');
    ...
    public function vectorStoreList(array $query = []): array
    {
        // Build query string
        $queryString = http_build_query($query);
        $url = '/vector_stores'.($queryString ? '?'.$queryString : '');
    ...
    public function vectorStoreListFiles(string $storeId, array $query = []): array
    {
        // Build query string
        $queryString = http_build_query($query);
        $url = "/vector_stores/{$storeId}/files".($queryString ? '?'.$queryString : '');
    ...
    public function vectorStoreFileMetadataUpdate(string $storeId, string $fileId, array $data): array
    {
        ...
        // Truncate attribute values that exceed 512 characters
        ...
        return $this->request('POST', "/vector_stores/{$storeId}/files/{$fileId}", [
