...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (DB::connection()->getDriverName() === 'pgsql') {
            // Check if pgvector is available by trying in a separate connection
            // to avoid poisoning the main transaction
            try {
                ...
                // pgvector might not be installed, that's ok - we'll use JSON column instead
                // This allows the application to run without pgvector for development/testing
                $pgvectorAvailable = false;
            }
        }
        ...
        Schema::create($tableName, function (Blueprint $table) use ($pgvectorAvailable) {
            ...
            if (DB::connection()->getDriverName() === 'pgsql' && $pgvectorAvailable) {
                ...
                // For other databases or PostgreSQL without pgvector, store as JSON (less efficient but compatible)
